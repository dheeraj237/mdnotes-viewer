...
describe('normalizeToFileNode', () => {
  test('raw with missing timestamps gets defaults', () => {
    ...
    const node = normalizeToFileNode(raw);
  ...
  test('directory raw with children array transforms correctly', () => {
    const raw = { id: 'd1', name: 'dir', path: '/dir', type: 'directory', children: ['f1', 'f2'] };
    const node = normalizeToFileNode(raw);
